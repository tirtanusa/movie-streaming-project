...
const MovieCard = (props) => {
  ...
  return (
    <>
      <div className="w-fit shrink-0 hover:scale-110 duration-300 transition-transform py-4 cursor-pointer">
        {/* Image */}
        <section>
          <img
            src={`https://image.tmdb.org/t/p/w780/${props.img_path}`}
            alt={props.title}
            className="w-[320px] h-[180px] object-cover rounded-lg bg-red-500"
          />
        </section>
        ...
        <section className="mt-2">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <p className="text-sm">{movieYear}</p>
              <div className="rounded-full bg-gray-600 w-1 h-1"></div>
              <p className="text-sm">{movieDuration} mins</p>
              <div className="rounded-full bg-gray-600 w-1 h-1"></div>
              <p className="text-sm">{genreNames}</p>
            </div>
          </div>
        </section>
      </div>
